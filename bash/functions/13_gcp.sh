...
gct() {
        ...
        TOKEN_FILE="/tmp/gct-${USER}-$(\ggrep -oP '(?<=account = ).*(?=$)' ~/.config/gcloud/configurations/config_default)/token"
        if [[ ! -e "$TOKEN_FILE" || "$(($(date '+%s') - $(gstat --printf='%Y' "$TOKEN_FILE")))" -ge "$VALIDITY_SECONDS" ]] ; then
