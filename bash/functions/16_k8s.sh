...
cluster_nodes() {
	...
	# Nodes Per AZ
	a=$(kubectl get node -l failure-domain.beta.kubernetes.io/zone=europe-west4-a | grep -v NAME | grep -v master | wc -l)
	b=$(kubectl get node -l failure-domain.beta.kubernetes.io/zone=europe-west4-b | grep -v NAME | grep -v master | wc -l)
	c=$(kubectl get node -l failure-domain.beta.kubernetes.io/zone=europe-west4-c | grep -v NAME | grep -v master | wc -l)
	d=$(kubectl get node -l failure-domain.beta.kubernetes.io/zone=europe-west4-d | grep -v NAME | grep -v master | wc -l)
	echo -e "\e[1m\e[39mWorker Nodes per AZ:\e[21m"
	echo -e "${BLUE}"europe-west4-a" \t :${GREEN}$a"
	echo -e "${BLUE}"europe-west4-b" \t :${GREEN}$b"
	echo -e "${BLUE}"europe-west4-c" \t :${GREEN}$c"
	echo -e "${BLUE}"europe-west4-d" \t :${GREEN}$d"
	#Node Types
